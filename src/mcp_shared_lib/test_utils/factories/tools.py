...
class MCPToolResultFactory(BaseFactory, SequenceMixin):
    ...
    def _create_success_result(cls, tool_name: str) -> Dict[str, Any]:
        ...
        if tool_name == 'analyze_repository':
            ...
            base_result['data'] = {
                ...
                'grouping_strategy': Faker.random_element(['by_feature', 'by_risk', 'by_type']),
    ...
    def _create_error_result(cls) -> Dict[str, Any]:
        ...
        error_types = [
            'ValidationError', 'FileNotFoundError', 'GitCommandError',
            'AnalysisError', 'NetworkError', 'TimeoutError', 'ConfigurationError'
        ]
        ...
        return {
            ...
            'error': {
                ...
                'cancellation_reason': Faker.random_element([
                    'user_request', 'system_shutdown', 'resource_limit', 'priority_override'
                ])
            }
        }
...
class MCPServerFactory(BaseFactory):
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        # Add available tools
        all_tools = [
            'analyze_repository', 'get_changes', 'assess_risk',
            'check_push_readiness', 'get_commit_history', 'analyze_diff',
            'recommend_prs', 'validate_changes', 'get_file_metadata',
            'calculate_metrics', 'generate_report', 'export_results'
        ]
...
class MCPClientFactory(BaseFactory):
    ...
    def user_agent() -> str:
        ...
        client_name = Faker.random_element(['mcp-client', 'repo-analyzer', 'dev-assistant'])
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        client['authentication'] = {
            ...
            'token_expires_at': datetime.now() + timedelta(days=Faker.random_int(1, 365)),
        ...
        client['session'] = {
            ...
            'last_activity': datetime.now() - timedelta(minutes=Faker.random_int(0, 60)),
...
class MCPTransactionFactory(BaseFactory):
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        transaction['duration_ms'] = int((response_time - request_time).total_seconds() * 1000)
        ...
        transaction['metrics'] = {
            'processing_time_ms': max(0, transaction['duration_ms'] - Faker.random_int(10, 100)),
...
def create_tool_execution_batch(
    ...
    if tool_names is None:
        tool_names = ['analyze_repository', 'get_changes', 'assess_risk', 'recommend_prs']
...
def create_server_farm(server_count: int = 3) -> Dict[str, Any]:
    ...
    return {
        'servers': servers,
        'load_balancer': {
            'strategy': Faker.random_element(['round_robin', 'least_connections', 'weighted']),
        ...
        'total_capacity': {
            'max_concurrent_requests': sum(s['max_concurrent_requests'] for s in servers),
            'total_tools': len(set(tool for s in servers for tool in s['available_tools'])),
...
def create_client_session_history(
    ...
    for i in range(total_requests):
        ...
        transaction = MCPTransactionFactory.create(
            request_timestamp=request_time
        )
        transactions.append(transaction)
    
    # Calculate session statistics
    successful_requests = len([t for t in transactions if t['response']['status_code'] == 200])
    total_bytes_transferred = sum(
        t['request']['size_bytes'] + t['response']['size_bytes'] 
        for t in transactions
    )
    average_response_time = sum(t['duration_ms'] for t in transactions) / len(transactions)
...
def create_performance_benchmark_data(
    ...
    for minute in range(duration_minutes):
        ...
        requests_this_minute = int(concurrent_clients * 6 * load_factor)  # ~6 req/min per client
    ...
    avg_response_time = sum(m['response_time_p50'] for m in metrics_per_minute) / len(metrics_per_minute)
    max_response_time = max(m['response_time_p99'] for m in metrics_per_minute)
    avg_error_rate = sum(m['error_rate_percent'] for m in metrics_per_minute) / len(metrics_per_minute)
    ...
    return {
        ...
        'summary_statistics': {
            ...
            'peak_memory_usage_mb': max(m['memory_usage_mb'] for m in metrics_per_minute),
            'peak_cpu_usage_percent': max(m['cpu_usage_percent'] for m in metrics_per_minute)
        },
        'performance_grade': _calculate_performance_grade(avg_response_time, avg_error_rate)
    }


def _calculate_performance_grade(avg_response_time: float, avg_error_rate: float) -> str:
