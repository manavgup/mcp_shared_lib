...
class AnalysisResultFactory(BaseFactory, SequenceMixin):
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        if result['status'] == 'success':
            result['summary'] = f"Analysis completed successfully. {result['files_analyzed']} files analyzed, {result['issues_found']} issues found."
        elif result['status'] == 'warning':
            result['summary'] = f"Analysis completed with {result['warnings_count']} warnings."
        elif result['status'] == 'error':
            result['summary'] = f"Analysis failed with {result['errors_count']} errors."
        else:
            result['summary'] = f"Analysis partially completed. {result['files_analyzed']} files processed."
...
class RiskAssessmentFactory(BaseFactory):
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        all_factors = [
            'large_change_size', 'critical_files_modified', 'low_test_coverage',
            'high_complexity', 'multiple_contributors', 'recent_bugs_in_area',
            'external_dependencies_changed', 'breaking_api_changes',
            'database_schema_changes', 'security_sensitive_code',
            'performance_critical_path', 'configuration_changes'
        ]
        
        # More factors for higher risk
        factor_count = min(len(all_factors), max(1, int(assessment['overall_score'] * 8)))
        ...
        assessment['mitigation_strategies'] = strategies or ['Monitor closely during deployment']
        
        # Generate impact analysis
        assessment['impact_analysis'] = {
            'affected_components': random.sample([
                'authentication', 'api', 'database', 'ui', 'processing',
            ...
            'rollback_difficulty': 'easy' if assessment['overall_score'] < 0.4 else 'medium' if assessment['overall_score'] < 0.8 else 'hard'
        }
...
def create_analysis_results(count: int = 1, status: str = None, **kwargs) -> List[Dict[str, Any]]:
    ...
    for _ in range(count):
        if status == 'success':
            ...
            result = AnalysisResultFactory.with_traits('analysis_with_warnings', **kwargs)
        elif status == 'error':
