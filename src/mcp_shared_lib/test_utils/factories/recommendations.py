...
class PRRecommendationFactory(BaseFactory, SequenceMixin):
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        rec['rationale'] = rationales.get(rec['pr_type'], 'Groups related changes together')
...
class RecommendationGroupFactory(BaseFactory):
    ...
    def group_name() -> str:
        ...
        return Faker.random_element([
            'Core Features', 'Bug Fixes', 'Documentation Updates',
            'Test Improvements', 'Performance Optimizations',
            'Security Enhancements', 'Configuration Changes'
        ])
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        group['total_files'] = sum(pr.get('file_count', 0) for pr in group['recommendations'])
        group['total_lines'] = sum(pr.get('total_lines_changed', 0) for pr in group['recommendations'])
        
        return group


# Convenience functions for creating recommendation collections
def create_pr_recommendation_set(count: int = 5, strategy: str = 'mixed') -> List[Dict[str, Any]]:
...
def create_recommendation_summary(recommendations: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        'average_files_per_pr': total_files / len(recommendations) if recommendations else 0,
        'average_lines_per_pr': total_lines / len(recommendations) if recommendations else 0,
