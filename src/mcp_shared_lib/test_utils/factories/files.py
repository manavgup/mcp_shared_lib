...
from .base import BaseFactory, Faker, create_realistic_file_content
...
class FileChangeFactory(BaseFactory):
    ...
    def file_path() -> str:
        ...
        if file_type == 'python':
            return Faker.file_path(depth=random.randint(2, 4), extension=extension.lstrip('.'))
            ...
            if random.random() < 0.5:
                ...
                return f"docs/{Faker.file_path(depth=1, extension=extension.lstrip('.'))}"
        else:
            return Faker.file_path(depth=random.randint(1, 3), extension=extension.lstrip('.'))
    ...
    def trait_high_risk(cls) -> Dict[str, Any]:
        ...
        critical_files = [
            ...
            'config/production.json', 'src/core/auth.py', 'migrations/001_initial.sql'
        ]
    ...
    def trait_low_risk(cls) -> Dict[str, Any]:
        ...
        safe_files = [
            'README.md', 'CHANGELOG.md', 'docs/api.md', 'docs/tutorial.md',
            'tests/test_utils.py', 'tests/fixtures/sample_data.json',
            '.gitignore', 'LICENSE'
        ]
    ...
    def trait_source_code(cls) -> Dict[str, Any]:
        ...
        source_files = [
            'src/analyzer.py', 'src/processor.py', 'src/utils.py',
            'src/models/base.py', 'src/services/git_service.py',
            'src/api/endpoints.py', 'src/cli/commands.py'
        ]
    ...
    def trait_test_file(cls) -> Dict[str, Any]:
        ...
        test_files = [
            'tests/test_analyzer.py', 'tests/test_processor.py',
            'tests/unit/test_models.py', 'tests/integration/test_workflow.py',
    ...
    def trait_documentation(cls) -> Dict[str, Any]:
        ...
        doc_files = [
            'README.md', 'docs/api.md', 'docs/tutorial.md', 'docs/setup.md',
    ...
    def trait_configuration(cls) -> Dict[str, Any]:
        ...
        config_files = [
            ...
            'docker-compose.yml', 'Dockerfile', '.github/workflows/test.yml',
            'config/development.json', 'config/production.yaml'
        ]
    ...
    **kwargs
    ...
    for i in range(count):
        ...
        if risk_distribution == "low":
            ...
            risk_type = Faker.weighted_choice(['low_risk', 'medium', 'high_risk'], [60, 30, 10])
        ...
        'largest_changes': [
            {
                ...
                'total_lines': change.get('lines_added', 0) + change.get('lines_removed', 0)
            }
...
def create_file_changes_by_category() -> Dict[str, List[Dict[str, Any]]]:
    ...
    return {
        'source_code': [
            FileChangeFactory.with_traits('source_code') for _ in range(Faker.random_int(2, 8))
        ],
        'tests': [
            FileChangeFactory.with_traits('test_file') for _ in range(Faker.random_int(1, 5))
        ],
        'documentation': [
            FileChangeFactory.with_traits('documentation') for _ in range(Faker.random_int(0, 3))
        ],
        'configuration': [
            FileChangeFactory.with_traits('configuration') for _ in range(Faker.random_int(0, 2))
        ]
    }