...
class SSETransport(HttpBasedTransport):
    ...
    def run(self, server: FastMCP) -> None:
        ...
        try:
            ...
            self.logger.info(f"Starting {self.server_name} with SSE transport on http://{host}:{port}")
            self.logger.debug(f"SSE endpoint will be available at /sse")
            ...
            try:
                ...
                try:
                    ...
                    self.logger.warning(f"streamable-sse failed, trying streamable-http: {e2}")
