...
class BaseTransport(ABC):
    ...
    def __init__(self, config: TransportConfig, server_name: str = "MCP Server"):
        ...
        self.logger = logging_service.get_logger(f"{__name__}.{self.__class__.__name__}")
        self._setup_logging()
...
class HttpBasedTransport(BaseTransport):
    ...
    def _setup_health_check(self, server: FastMCP) -> None:
        ...
        if getattr(transport_config, "enable_health_check", False):
            ...
            async def health_check() -> str:
                ...
                return f"Server Status: {status['status']}\n" \
    ...
    def is_running(self) -> bool:
        ...
        return self._is_running and (self._server_process is None or self._server_process.poll() is None)
